...
class DocManagerBase(object):
    ...
    def insert_file(file, ns):
        pass

    def remove_file(file, ns):
        pass
...
class GridFSManager:
    ...
    def add_fs(self, ns):
        self.files_ns.append(ns + '.files')
        self.chunks_ns.append(ns + '.chunks')
    ...
    def upsert(self, doc):
        ...
        if ns in self.files_ns:
            k = fs + str(doc['_id'])
            f = self.active.get(k, {})
            f['num_chunks'] = (doc['length'] - 1) / doc['chunkSize'] + 1
            f['meta'] = doc
            self.active[k] = f
            result = True

        if ns in self.chunks_ns:
            k = fs + str(doc['files_id'])
            f = self.active.get(k, {})
            chunks = f.get('chunks', [])
            chunks.append(doc)
            f['chunks'] = chunks
        ...
        if result:
            logging.debug(f)
            if 'chunks' in f and 'num_chunks' in f:
                if len(f['chunks']) == f['num_chunks']:
                    self.assemble(f)
                    del self.active[k]
    ...
    def remove(self, document):
        ...
        if ns + '.files' in self._files_ns:
            # remove the file
            k = ns + doc['_id']
            del self._active[k]
            # tell the doc manager to remove the file
            return True

        if ns + '.chunks' in self._chunks_ns:
            return True
        
        return False

    def assemble(self, document):
        logging.debug("GRIFS ASSEMBLE!")

        print(type(document['chunks'][0]['data']))
        document['chunks'] = [len(x) for x in document['chunks']]
        document['chunks'].append("total: %d", sum(document['chunks']))
